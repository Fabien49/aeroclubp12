...
public class BorrowController {
    ...
    public List<Reservation> getBorrows() {

        logger.info("Providing borrow resource from database: all borrow list");

        List<Reservation> borrows = borrowManager.getAllBorrows();

        return borrows;
    }

    @GetMapping(value="/borrows/{id}")
    public Reservation getBorrowById(@PathVariable @Min(value = 1) int id) {

        logger.info("Providing borrow resource from database: borrow id: " + id);

        Optional<Reservation> borrow = borrowManager.getById(id);
    ...
    @Transactional
    public ResponseEntity<Void> addBorrow(@Valid @RequestBody Reservation borrow) {

        logger.info("Adding new borrow in database");
          
        Reservation borrowAdded;
        try {
...
/* // Init Ids
    ...
    public ResponseEntity<Void> updateBorrow(@PathVariable @Min(value = 1) int id, @Valid @RequestBody Reservation borrowDetails) throws FunctionnalException {
    ...
    public ResponseEntity<Void> extendBorrow(@PathVariable @Min(value = 1) int id, @Valid @RequestBody Reservation borrowDetails) {
...
/* // Init Ids
    ...
    public List<Reservation> getBorrowsByUserId(@PathVariable(value = "user_id") @Min(value = 1) int userId) {

        logger.info("Providing borrow resources from database by user id: " + userId);

        List<Reservation>  borrows = borrowManager.getAllBorrowsByRegistereduserId(userId);
